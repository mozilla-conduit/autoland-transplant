...
def main():
    try:
        ...
        # post-job
        cmd = subparsers.add_parser('post-job',
                                    help='Post job to autoland')
        ...
        cmd.add_argument('--pingback-url', default='http://localhost:9898/',
                         help='URL to which Autoland should post result')
        cmd.add_argument('--trysyntax',
                         help='Try syntax to use')
        cmd.add_argument('--push-bookmark',
                         help='Bookmark name to use when pushing')
        cmd.add_argument('--commit-descriptions',
                         help='Commit descriptions to use when rewriting')
        cmd.add_argument('--username', default='autoland',
                         help='autoland api username')
        cmd.add_argument('--password', default='autoland',
                         help='autoland api password')
        cmd.add_argument('--patch-url',
                         help='URL of patch')
        cmd.add_argument('--patch-file',
                         help='Patch file to inline into request')
        cmd.set_defaults(func=post_job)

        # job-status
        cmd = subparsers.add_parser('job-status',
                                    help='Get an autoland job status')
        cmd.add_argument('request_id',
                         help='ID of the job for which to get status')
        cmd.add_argument('--poll', action='store_true',
                         help='Poll the status until the job is serviced')
        cmd.set_defaults(func=job_status)

        # exec
        cmd = subparsers.add_parser('exec',
                                    help='Execute command within a container')
        cmd.add_argument('--container', '-c', default='daemon',
                         help='Container name (will be prefixed with '
                              '"autoland_test.")')
        cmd.add_argument('--user',
                         help='User to execute commands as')
        cmd.add_argument('--shell', '-s', action='store_true', default=False,
        ...
        # treestatus
        cmd = subparsers.add_parser('treestatus',
                                    help='Set the treestatus response')
        cmd.add_argument('status', choices=['open', 'closed'],
                         help='Status')
